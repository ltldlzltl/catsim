...

{
  ...
  for(i=0;i<3;i++) axes[0][i] /= nrm[0];  //normalize main
  for(i=0;i<3;i++)
    dbug(3,"%1.10f --------------------*******************************************************************\n",axes[0][i]);
  // for(i=0;i<3;i++) printf("%1.15f\n",axes[0][i]);
  dot = DOT(axes[0],axes[1]);
  for(i=0;i<3;i++) axes[1][i] -= dot * axes[0][i];  // orthogonalize
  nrm[1] = 0;
  for(i=0;i<3;i++) nrm[1] += axes[1][i] * axes[1][i];
  nrm[1] = sqrt(nrm[1]);
  for(i=0;i<3;i++) axes[1][i] /= nrm[1];  //normalize 2nd
  // for(i=0;i<3;i++) printf("%1.15f\n",axes[1][i]);
...
{
  if (treepointer->left != NULL) FreeItem_BVH(treepointer->left);
  if (treepointer->right != NULL) FreeItem_BVH(treepointer->right);
...

{
  ...
  */
  ...
  for(i=0;i<critical_pts.length;i++) {
    ...
    if (critical_pts.sp[i].sense == 0) {  //start of a segment
      this = 0;
...
int isAwayFromPatch(double patch[4][4][3], float position[3], double my_tol)
{
  ...
  if (!Check_IntPoint(p_ext, position[0], position[1], position[2], my_tol))
	...
	return (   isAwayFromPatch(ul_patch, position, my_tol)
		&& isAwayFromPatch(ur_patch, position, my_tol)
		&& isAwayFromPatch(dl_patch, position, my_tol)
		&& isAwayFromPatch(dr_patch, position, my_tol));
...
// Note that when the distance is between my_tol and ~3*my_tol, the output can be either 0 or 1!
{
  ...
  p_ext[0][0] = treepointer->xmin;  p_ext[0][1] = treepointer->xmax;
  p_ext[1][0] = treepointer->ymin;  p_ext[1][1] = treepointer->ymax;
  p_ext[2][0] = treepointer->zmin;  p_ext[2][1] = treepointer->zmax;

  if (Check_IntPoint(p_ext, position[0], position[1], position[2], my_tol)){ 
    if(treepointer->right != NULL)
      if (!isAwayFromBez(treepointer->right, bez_model, position, my_tol)) return 0;
    if(treepointer->left != NULL)
      if (!isAwayFromBez(treepointer->left, bez_model, position, my_tol)) return 0;
    if(treepointer->right == NULL && treepointer->left == NULL)
      for(i = 0; i < treepointer->num; i++){
	if (!isAwayFromPatch(bez_model->patches[treepointer->patches[i]].cpoints, position, my_tol)) return 0;
      }
  }
  return 1;
}
    ...
    if(treepointer->right == NULL && treepointer->left == NULL)
	  ...
	  if (xform_patches)
	  ...
	  else
	    {
	      ...
	      for(j=0;j<4;j++)
		for(k=0;k<3;k++)
		  if (j==k) xform2[j][k] = 1; else xform2[j][k] = 0;
	  ...
	  if (int_points->length > ilo) {
	    ...
	    if (debug_flag >= 2) patch_in_matlab(bez_model->patches[treepointer->patches[i]].cpoints);
	  }
	}
    //dbug(2,"fend1\n");
    }
  ...
  while (cnt < 50)  //usually will return on first iteration (very rarely will take more than 2 or 3)
    {
      ...
      if (isAwayFromBez(treepointer[organ_id], &bez_model[organ_id], vec, my_tol)) return x;  //usually done 1st time here
...
{ 
  ...
  */
  ...
  while (!done) {
    ...
    for(i = 0; i < 3; i++) line_v[i] = mdps[cnt][i];
    cnt++;
...
int Check_Y_Boundary(float x, float y, float z, int organ_id)
{
  ...
  line_o[0] = x; line_o[1] = y; line_o[2] = z;
  ...
  for(cnt=0;cnt<6;cnt++)
    {
      dbug(3,"cnti: %d\n",cnt);
      line_v[0] = 0.0; line_v[1] = 0.0; line_v[2] = 0.0;
      if (cnt<=2)
        line_v[cnt%3] = 1.0;
      else
        line_v[cnt%3] = -1.0;
  ...
  for(cnt=0;cnt<6;cnt++)
    {
      line_v[0] = 0.1; line_v[1] = 0.1; line_v[2] = 0.1;
      if (cnt<=2)
        line_v[cnt%3] = 1.0;
      else
        line_v[cnt%3] = -1.0;
...
int Check_Y_Boundary_tri(bvh_element *treepointer, TRI_MODEL tmodel, float x, float y, float z, int organ_id)
{
  ...
  line_o[0] = x; line_o[1] = y; line_o[2] = z;
  line_v[0] = 0; line_v[1] = 1.0; line_v[2] = 0.0;
...

{
  ...
  for(i=0;i<11;i++)
	...
	if (dist<2.1) {edges[n][0]=i;edges[n][1]=j;n++;}
      }
  dbug(1,"EDGES:                    %d                 ----------------\n",n);
      ...
      for(k=j+1;k<12;k++)
	{
	  ...
	  if (dist<2.1) {
		...
		disp[l] = mdps[k][l] - mdps[i][l];
	      dist = sqrt(DOT(disp,disp));
	      if (dist<2.1) {faces[n][0]=i;faces[n][1]=j;faces[n][2]=k;n++;}
	    }
	  }
	}
  dbug(1,"FACES:                    %d                 ----------------\n",n);
  ...
  R1[1][0] = 0; R1[1][1] = c; R1[1][2] = -s;
  R1[2][0] = 0; R1[2][1] = s; R1[2][2] = c;

  R2[0][0] = c; R2[0][1] = -s; R2[0][2] = 0;
  R2[1][0] = s; R2[1][1] = c; R2[1][2] = 0;
  R2[2][0] = 0; R2[2][1] = 0; R2[2][2] = 1;

  for(i=0;i<3;i++)
    matrix_mult(R1,R2[i],R[i]);

  R2[0][0] = c; R2[0][1] = 0; R2[0][2] = s;
  R2[1][0] = 0; R2[1][1] = 1; R2[1][2] = 0;
  R2[2][0] = -s; R2[2][1] = 0; R2[2][2] = c;
          ...
          if (segments.length > 0)
            {
              ...
              while((segments.sp[p].x2 < xbrd) && (p < segments.length)) p++;
  ...
  for(l = 0;l<nSubDets;l++)
      ...
      for(k = 0;k<modules_NCAT.nSubSources;k++)
	{
	  ...
	  // Now we do the final calculations for thisView
	      ...
	      if (0) {  // use this to sanity test Break_segment2() vs Break_segment()
